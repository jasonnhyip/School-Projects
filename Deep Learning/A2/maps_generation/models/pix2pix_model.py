...
class Pix2PixModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='unet_256', dataset_mode='map')
        if is_train:
    ...
    def __init__(self, opt):
        ...
        # if opt.netG == 'res_unet_G_seg':
        #     self.netS = smp.Unet(encoder_name="vgg16", encoder_weights="imagenet", in_channels=3, classes=1,)
        ...
        if not opt.isTrain:
            print(opt.name)
            self.network_loading(self.netG, './train_output/ckpt/' + opt.name + '/' + opt.epoch + '_net_G.pth')
            self.netG.eval()
            self.set_requires_grad([self.netG], False)

        self.update_D = True

    # def semantic_output(self, image):
    #     return self.netS(image)
